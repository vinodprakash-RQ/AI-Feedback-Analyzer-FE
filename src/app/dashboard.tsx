...
const navItems = [{ label: 'Overview', href: '/', icon: LayoutDashboard }, { label: 'Issue list', href: '/issues', icon: Inbox }];
...
export default function Dashboard() {
  ...
  return <div className="app-shell">
    <aside className={`sidebar ${mobileNav ? 'mobile-open' : ''}`}>
      ...
      <div className="workspace-switcher"><div className="workspace-icon">RA</div><div><strong>Revolte.AI</strong><span>Product workspace</span></div><ChevronDown size={15} /></div>
      <nav aria-label="Main navigation"><p className="nav-label">WORKSPACE</p>{navItems.map(({ label, href, icon: Icon }) => <Link key={label} href={href} className={`nav-item ${label === 'Overview' ? 'active' : ''}`}><Icon size={17} /><span>{label}</span>{label === 'Issue list' && <em>{issueCount}</em>}</Link>)}</nav>
      <div className="sidebar-bottom"><div className="profile"><Avatar initials="VP" /><div><strong>VInod Prakash</strong><span>Admin</span></div><MoreHorizontal size={16} /></div></div>
    </aside>
    ...
    <main className="main-content">
      ...
      <div className="page-wrap">
        ...
        {viewState === 'loading' ? <StatePanel type="loading" /> : viewState === 'error' ? <StatePanel type="error" onRetry={() => { setViewState('loading'); setRetryKey((value) => value + 1); }} /> : viewState === 'empty' ? <StatePanel type="empty" onRetry={() => { setViewState('loading'); setRetryKey((value) => value + 1); }} /> : <>
          ...
          <section className="panel timeline-panel"><div className="panel-heading"><div><h2>Issues reported over time</h2><p>Incoming issue volume across the last 30 days</p></div><button className="select-button">Last 30 days <ChevronDown size={14} /></button></div><TimelineChart issues={issues} /></section>
...
function TimelineChart({ issues }: { issues: Issue[] }) { const dates = issues.map((issue) => issue.createdAt ? new Date(issue.createdAt) : null).filter((date): date is Date => Boolean(date && !Number.isNaN(date.getTime()))); const start = dates.length ? Math.min(...dates.map((date) => date.getTime())) : Date.now(); const end = dates.length ? Math.max(...dates.map((date) => date.getTime())) : start; const range = Math.max(end - start, 86400000); const buckets = Array.from({ length: 6 }, (_, index) => ({ date: new Date(start + range * index / 5), value: 0 })); dates.forEach((date) => { const index = Math.min(5, Math.floor((date.getTime() - start) / (range / 6))); buckets[index].value += 1; }); const max = Math.max(1, ...buckets.map((bucket) => bucket.value)); const points = buckets.map((bucket, index) => `${index * 180},${190 - bucket.value / max * 165}`).join(' '); return <div className="timeline-chart"><div className="timeline-y"><span>{max}</span><span>{Math.ceil(max * .66)}</span><span>{Math.ceil(max * .33)}</span><span>0</span></div><div className="timeline-area"><div className="grid-lines"><i /><i /><i /><i /></div><svg viewBox="0 0 900 210" preserveAspectRatio="none" role="img" aria-label="Issues reported over time"><polyline points={`0,210 ${points} 900,210`} fill="#3468f51f" stroke="none" /><polyline points={points} fill="none" stroke="#3468f5" strokeWidth="3" strokeLinejoin="round" strokeLinecap="round" /></svg><div className="x-axis">{buckets.map((bucket) => <span key={bucket.date.toISOString()}>{bucket.date.toLocaleDateString('en', { month: 'short', day: 'numeric' })}</span>)}</div></div></div> }
