...
import { fetchIssuePage, updateIssueStatus } from '@/lib/feedback-api';
...
export default function IssuesClient() {
  ...
  const handleStatusChange = async (nextStatus: Status) => { if (!selected || nextStatus === selected.status) return; setStatusUpdating(true); setStatusError(''); try { await updateIssueStatus(selected.id, nextStatus); setIssues((current) => current.map((issue) => issue.id === selected.id ? { ...issue, status: nextStatus } : issue)); setSelected((current) => current ? { ...current, status: nextStatus } : current); } catch (error) { setStatusError(error instanceof Error ? error.message : 'Could not update the issue status.'); } finally { setStatusUpdating(false); } };
  ...
  return <div className="app-shell issues-app">
    ...
    {selected && <IssueDetails issue={selected} onClose={() => { setSelected(null); setStatusError(''); }} onStatusChange={handleStatusChange} statusUpdating={statusUpdating} statusError={statusError} />}
  </div>;
}
