...
export function backendHeaders(request: Request, includeJson = false, apiKeyEnv = 'FEEDBACK_API_KEY') { const headers = new Headers({ Accept: 'application/json', 'X-Request-ID': request.headers.get('X-Request-ID') || randomUUID() }); if (includeJson) headers.set('Content-Type', 'application/json'); const apiKey = process.env[apiKeyEnv]; if (apiKey) headers.set('x-api-key', apiKey); return headers; }
